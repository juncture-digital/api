...
from mdrender.gh import has_gh_repo_prefix, _get_gh_file, put_gh_file
...
class Annotations(object):
  ...
  def __init__(self, **kwargs):
    self.annotations_store = s3.Bucket (bucket='ve-annotations') if default_md_store == 's3' else gcs.Bucket(bucket='ve-annotations')
  ...
  def _token_is_valid(self, claims):
    if 'exp' in claims:
      ...
      logger.debug(f'_token_is_valid: current={current} expires={claims["exp"]} remaining={remaining}')
      return remaining > 0
    return False

  def _set_annotations(self, user, repo, target, annotations, sha, token):
    logger.info(f'_set_annotations: user={user} repo={repo} target={target} sha={sha} token={token}')
    if repo:
  ...
  def get_annotations(self, path, base_url):
    logger.info(f'get_annotations: path={path} base_url={base_url}')
    ...
    anno_base_uri = f'{base_url if base_url else "/"}annotation{target_uri}'
    ...
    return {'user': user, 'repo': repo, 'target': target, 'annotations': annotations}

  def create_annotation(self, path, annotation, token, **kwargs):
    logger.info(f'create_annotation: path={path}')
    user, repo, target = self._parse_path(path)
  ...
  def get_annotation(self, path, base_url):
    ...
    logger.debug(f'get_annotation: user={user} target={target} annoid={annoid}')
    annotations, _ = self._get_annotations(user, repo, target, base_url)
    if len(annotations) == 0 and '/' in target:
      annotations, _ = self._get_annotations(user, repo, target.split('/')[-1], base_url)
  ...
  def delete_annotation(self, path, token):
    ...
    annoid = target.split('/')[-1]
    target = '/'.join(target.split('/')[:-1])
    logger.info(f'delete_annotation: path={path} user={user} repo={repo} target={target} annoid={annoid}')
    annotations, sha = self._get_annotations(user, repo, target)
  ...
  def update_annotation(self, path, annotation, token):
    ...
    target = '/'.join(target.split('/')[:-1])
    logger.info(f'update_annotation: user={user} repo={repo} target={target} annoid={annoid} token={token}')
    annotations, sha = self._get_annotations(user, repo, target)
    for i in range(len(annotations)):
      if annotations[i]['id'].split('/')[-1] == annoid:
